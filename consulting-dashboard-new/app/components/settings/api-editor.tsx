...
export function ApiEditor() {
  ...
  const loadContent = async () => {
    ...
    setSaveMessage(null)
    try {
      const result = await getApiContent(selectedService)
      if (result.success && result.data) {
        setContent(result.data)
        setOriginalContent(result.data)
      } else {
        // ファイルが存在しない場合は空のテンプレートを設定
        const template = `# ${services.find(s => s.value === selectedService)?.label} API仕様

## 基本情報
- ベースURL: \`/api/${selectedService.replace('-service', '')}\`
- 認証: JWT Bearer Token
- Content-Type: application/json

## エンドポイント一覧

### GET /api/${selectedService.replace('-service', '')}/[endpoint]
[エンドポイントの説明]
...
`
  ...
  const handleSave = async () => {
    ...
    setSaveMessage(null)
    try {
      const result = await saveApiContent(selectedService, content)
      if (result.success) {
        ...
        <Tabs value={viewMode} onValueChange={(v) => setViewMode(v as 'edit' | 'preview')}>
          ...
          <TabsContent value="edit" className="mt-4">
            <Textarea
              ...
              placeholder="Markdown形式でAPI仕様を記述..."
            />
          </TabsContent>
