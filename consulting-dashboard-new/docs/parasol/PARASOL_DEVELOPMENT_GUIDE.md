...
| 概念体系 | 主要概念 | 空間/レベル | 目的 | 焦点 | 担当者 | 成果物 |
|---------|---------|-----------|------|------|--------|--------|
| **ビジネスの階層**<br/>（バリューステージ構造） | **バリューストリーム**<br/>**バリューステージ**<br/>**ビジネスケーパビリティ** | - | 何をするか（What） | ビジネス価値・組織能力 | ビジネス部門<br/>経営層 | バリューストリームマップ<br/>ケーパビリティマップ |
| **DDDの階層**<br/>（問題領域→解決領域） | **Subdomain**（問題領域）<br/>**Bounded Context**（解決領域） | **問題領域**: Subdomain<br/>**解決領域**: BC | どうモデル化するか（How to Model） | ドメイン知識の構造化 | ドメインエキスパート<br/>アーキテクト | サブドメインマップ<br/>ドメインモデル<br/>コンテキストマップ |
| **実装の階層**<br/>（ソリューション構成） | **Microservice**<br/>**Service Module** | **ソリューション構成**: MS | どう実装・構成するか（How to Implement） | システムの物理構造 | エンジニア<br/>DevOpsチーム | ソースコード<br/>API仕様<br/>インフラ構成 |
...
| レベル | 名称 | 説明 | 例 |
|--------|------|------|-----|
| **B0** | **バリューストリームレベル** | 顧客への価値の流れ全体 | "DXプロジェクト成功支援" |
| **B1** | **バリューステージレベル** | 価値創造の各段階 | "プロジェクト計画支援"、"実行支援"、"成果可視化" |
| **B2** | **ケーパビリティL1レベル** | 戦略的組織能力 | "プロジェクト構想する能力" |
| **B3** | **ケーパビリティL2レベル** | 戦術的組織能力 | "プロジェクト実行する能力" |
| **B4** | **ケーパビリティL3レベル** | 具体的な業務能力 | "タスクを管理する能力" |
| **B5** | **プロセスレベル** | 具体的なビジネスプロセス | "ユーザー認証プロセス" |
| **B6** | **アクティビティレベル** | エンドユーザーの活動 | "パスワードでログインする" |
...
**作成手順**:
1. VALUE_STREAM_MAP.mdから各ステージを抽出
2. 各ステージの価値提供ポイント、成果指標を定義
3. AIに各ステージの詳細化を依頼
4. ステージ間の依存関係を整理
...
**作成手順**:
1. 各バリューステージ内のケーパビリティを分析
2. L1（戦略）→ L2（戦術）→ L3（具体）の階層化
...
**作成手順**:
1. 各ケーパビリティL1に対応するサブドメインを抽出
2. Core/Supporting/Genericのタイプ判定
3. AIに戦略的な判断基準を提案させる
4. ドメインエキスパートと合意形成
...
**作成手順**:
1. Subdomain → Bounded Contextのマッピング決定
2. 各BCのドメインモデル設計（Aggregate, Entity, VO）
3. AIにドメインモデルのドラフトを依頼
4. コンテキストマップ（BC間の関係）を作成
...
**作成手順**:
1. BC → Microserviceのマッピング決定
2. デプロイメント戦略（モノリス/MS/ハイブリッド）を選択
...
**AI協働のベストプラクティス**:
```
1. ステップ0-3: AIに「[業務ドメイン]のサブドメインマップを作成して」と依頼
   → AIがドラフトを生成
   → ビジネス部門がレビュー・調整

2. ステップ4: AIに「[Subdomain名]のBounded Contextを設計して」と依頼
   → AIがドメインモデルを提案
   → アーキテクトがレビュー・精緻化

3. ステップ5: AIに「[BC名]のREST API仕様を生成して」と依頼
   → AIが実装仕様を生成
   → エンジニアが実装
