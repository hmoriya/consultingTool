...
**各ステップの成果物**:
```
ステップ0 → VALUE_STREAM_MAP.md（バリューストリームマップ）
ステップ1 → VALUE_STAGE_DEFINITION.md（バリューステージ定義）
ステップ2 → CAPABILITY_MAP.md（ケーパビリティマップ）
ステップ3 → SUBDOMAIN_MAP.md（サブドメインマップ：Core/Supporting/Genericの分類）
ステップ4 → context.md, context-map.md, domain-language.md
ステップ5 → api-specification.md, database-design.md, docker-compose.yml
```
