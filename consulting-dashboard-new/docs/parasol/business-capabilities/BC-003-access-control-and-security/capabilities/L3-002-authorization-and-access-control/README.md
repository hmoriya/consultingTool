...
#### 推奨ライブラリ・フレームワーク
- **RBAC**: [accesscontrol](https://github.com/onury/accesscontrol) - ロールベースアクセス制御
- **CASL**: [CASL](https://casl.js.org/) - 属性ベースアクセス制御（ABAC）
- **権限管理**: カスタムRBACエンジン（ロール階層サポート）
...
#### キャッシュ戦略
- **ユーザー権限**: Redis cache（TTL: 15分、ロール変更時に無効化）
- **ロール定義**: アプリケーションメモリ（起動時ロード、変更時リロード）
- **リソースACL**: Redis cache（TTL: 10分）
