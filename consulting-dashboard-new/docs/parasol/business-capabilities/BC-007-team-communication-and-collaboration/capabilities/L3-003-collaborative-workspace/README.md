# L3-003: Collaborative Workspace

**作成日**: 2025-10-31
**所属BC**: BC-007: Team Communication & Collaboration
**V2移行元**: provide-collaborative-environment

---

## 📋 What: この能力の定義

### 能力の概要
協働作業のための共有環境を提供する能力。共有ワークスペース、ドキュメント共有、共同編集を通じて、チームコラボレーションを促進します。

### 実現できること
- 共有ワークスペースの作成と管理
- ドキュメント共同編集
- ファイル共有と権限管理
- リアルタイム同期
- バージョン履歴管理

### 必要な知識
- コラボレーション技術
- ドキュメント管理
- 同期技術
- アクセス制御
- バージョン管理

---

## 🔗 BC設計の参照（How）

### ドメインモデル
- **Aggregates**: WorkspaceAggregate ([../../domain/README.md](../../domain/README.md#workspace-aggregate))
- **Entities**: Workspace, SharedDocument, CollaborationSession, Version
- **Value Objects**: WorkspaceType, AccessPermission, SyncStatus

### API
- **API仕様**: [../../api/api-specification.md](../../api/api-specification.md)
- **エンドポイント**:
  - POST /api/workspaces - ワークスペース作成
  - POST /api/workspaces/{id}/documents - ドキュメント共有
  - PUT /api/documents/{id}/collaborate - 共同編集

詳細: [../../api/README.md](../../api/README.md)

### データ
- **Tables**: workspaces, shared_documents, collaboration_sessions, document_versions

詳細: [../../data/README.md](../../data/README.md)

---

## 🛠️ 実装アプローチ

### アーキテクチャパターン
- **リアルタイムコラボレーション**: CRDT/OT による競合解決
- **ドキュメント管理**: バージョン管理 + 差分ベース保存
- **権限管理**: RBAC による細粒度アクセス制御
- **活動追跡**: イベントソーシングによる変更履歴

### 主要技術スタック
- **リアルタイム同期**: CRDT (Conflict-free Replicated Data Types)
  - Yjs / Automerge（CRDT実装）
  - WebSocket による変更配信
  - 自動マージ（競合なし）
  - オフライン対応（後で同期）
- **OT (Operational Transformation)**: ShareDB
  - Google Docs スタイルの同時編集
  - 変更操作の変換・適用
  - 一貫性保証
- **ドキュメントストレージ**: S3/MinIO + PostgreSQL
  - ドキュメント本体: オブジェクトストレージ
  - メタデータ: PostgreSQL
  - バージョン履歴: 差分保存
- **検索・インデックス**: Elasticsearch
  - ドキュメント全文検索
  - メタデータ検索
  - ファセット検索（ワークスペース別）
- **WebSocket サーバー**: Socket.io / Y-WebSocket
  - リアルタイム編集同期
  - カーソル位置共有
  - 編集中ユーザー表示
  - プレゼンス管理

### 実装パターン

#### CRDT によるリアルタイム同期
```typescript
// Yjs CRDT 実装例
import * as Y from 'yjs'
import { WebsocketProvider } from 'y-websocket'

// ドキュメント初期化
const ydoc = new Y.Doc()
const ytext = ydoc.getText('content') // 共有テキスト

// WebSocket接続（リアルタイム同期）
const wsProvider = new WebsocketProvider(
  'ws://localhost:1234',
  'document-123',
  ydoc
)

// ローカル編集
ytext.insert(0, 'Hello ')
ytext.insert(6, 'World')

// 他ユーザーの変更を自動マージ
ydoc.on('update', (update) => {
  // 変更を自動反映（競合なし）
  const currentText = ytext.toString()
  console.log('Current text:', currentText)
})
```

#### OT によるリアルタイム編集
```typescript
// ShareDB OT 実装例
import ShareDB from 'sharedb'

const connection = new ShareDB.Connection(socket)
const doc = connection.get('documents', 'doc-123')

// ドキュメント購読
doc.subscribe((err) => {
  if (err) throw err

  // ローカル編集
  doc.submitOp([
    { p: ['content', 0], si: 'Hello ' } // 挿入操作
  ])

  // 他ユーザーの変更を受信
  doc.on('op', (op, source) => {
    if (!source) {
      // リモート操作 → UI更新
      applyOperationToEditor(op)
    }
  })
})
```

#### バージョン管理システム
```typescript
// ドキュメントバージョン管理
class DocumentVersionManager {
  async saveVersion(documentId, content, userId) {
    const previousVersion = await this.getLatestVersion(documentId)

    // 差分計算（diff-match-patch）
    const diffs = this.diffAlgorithm.diff(
      previousVersion.content,
      content
    )

    // バージョン保存
    const version = await this.db.documentVersions.create({
      documentId,
      versionNumber: previousVersion.versionNumber + 1,
      diffs: this.diffAlgorithm.toDelta(diffs),
      authorId: userId,
      createdAt: new Date()
    })

    return version
  }

  async restoreVersion(documentId, versionNumber) {
    // 指定バージョンまでの差分を適用
    const versions = await this.db.documentVersions.findAll({
      where: { documentId, versionNumber: { lte: versionNumber } },
      order: [['versionNumber', 'ASC']]
    })

    let content = ''
    for (const version of versions) {
      content = this.diffAlgorithm.applyDelta(content, version.diffs)
    }

    return content
  }
}
```

#### 権限管理（RBAC）
```typescript
// ワークスペース権限管理
const PERMISSIONS = {
  owner: ['read', 'write', 'delete', 'invite', 'manage'],
  editor: ['read', 'write', 'comment'],
  commenter: ['read', 'comment'],
  viewer: ['read']
}

async function checkPermission(userId, workspaceId, action) {
  const member = await db.workspaceMembers.findOne({
    where: { userId, workspaceId }
  })

  if (!member) throw new Error('Not a workspace member')

  const allowedActions = PERMISSIONS[member.role]
  if (!allowedActions.includes(action)) {
    throw new Error(`Permission denied: ${action}`)
  }

  return true
}
```

### パフォーマンス最適化
- **リアルタイム同期最適化**:
  - 変更バッチング: 複数変更を100msごとにまとめて送信
  - 圧縮: WebSocket メッセージの gzip 圧縮
  - Debounce: 高頻度変更の間引き
- **ドキュメント読み込み**:
  - 遅延読み込み: 大ドキュメントの段階的ロード
  - キャッシュ: Redis による頻繁アクセスドキュメントキャッシュ
  - CDN: 静的ファイルの配信
- **バージョン履歴**:
  - 差分圧縮: 古いバージョンの圧縮保存
  - アーカイブ: 90日以上の履歴をアーカイブストレージへ
- **スケーリング**:
  - 水平スケーリング: 複数WebSocketサーバー + Redis Pub/Sub
  - 読み取り分散: ドキュメント読み取りレプリカ

---

## ⚠️ 前提条件と制約

### 技術的前提条件
- **インフラ要件**:
  - WebSocket サーバー（Node.js: 4GB RAM/サーバー）
  - Redis（CRDT/OT 状態同期）
  - PostgreSQL（メタデータ・権限管理）
  - S3/MinIO（ドキュメントストレージ）
  - Elasticsearch（全文検索）
- **クライアント要件**:
  - モダンブラウザ（Chrome 90+, Firefox 88+, Safari 14+）
  - WebSocket サポート必須
  - JavaScript 有効
- **リソース要件**:
  - 最低: 8GB RAM, 4 vCPU（同期サーバー）
  - 推奨: 16GB RAM, 8 vCPU（高負荷環境）

### ビジネス制約
- **ワークスペース制限**:
  - メンバー数: 最大100人/ワークスペース
  - ドキュメント数: 最大1000件/ワークスペース
  - ストレージ: 100GB/ワークスペース（有料プランは無制限）
- **ドキュメント制限**:
  - サイズ: 最大10MB/ドキュメント
  - 同時編集: 最大10人/ドキュメント
  - バージョン履歴: 最大100世代（それ以降は圧縮）
- **リアルタイム同期**:
  - 同期遅延: < 100ms（95パーセンタイル）
  - 競合解決: 自動（CRDT/OT）
  - オフライン編集: 24時間以内に同期

### データ制約
- **ドキュメント形式**: Markdown, Rich Text, Code, Spreadsheet
- **添付ファイル**: 最大10個、合計50MB/ドキュメント
- **バージョン保持**: 90日間（有料プランは無制限）
- **検索インデックス**: 全ドキュメント即時インデックス

### 運用制約
- **可用性**: 99.9%（年間ダウンタイム < 8.76時間）
- **バックアップ**: 日次自動バックアップ
- **監視**: リアルタイム編集数、同期遅延、WebSocket接続数
- **セキュリティ**: E2E暗号化（オプション）、TLS 1.3必須

---

## 🔗 BC設計との統合

### ドメインモデル統合
- **Aggregate Root**: `WorkspaceAggregate`
  - 集約エンティティ: Workspace, SharedDocument, CollaborationSession, Version
  - 不変条件: ワークスペースメンバー最大数、同時編集上限、権限整合性
  - ビジネスルール: アクセス制御、ドキュメント共有ポリシー、バージョン管理

### BC間連携

#### BC-001 (Project Delivery) との連携
- **プロジェクトワークスペース自動作成**:
  - イベント: `ProjectCreatedEvent`
  - アクション: プロジェクト専用ワークスペース + テンプレートドキュメント自動生成
  - API: `POST /api/workspaces/for-project/{projectId}`
- **成果物ドキュメント連携**:
  - イベント: `DeliverableCreatedEvent`
  - アクション: ワークスペースに成果物ドキュメント追加
  - API: `POST /api/workspaces/{id}/documents/from-deliverable`

#### BC-005 (Team & Resource) との連携
- **チームワークスペース**:
  - イベント: `TeamCreatedEvent`
  - アクション: チーム専用ワークスペース自動作成
  - API: `POST /api/workspaces/for-team/{teamId}`
- **メンバー同期**:
  - イベント: `TeamMemberAddedEvent`, `TeamMemberRemovedEvent`
  - アクション: ワークスペースメンバー自動追加/削除
  - API: `POST /api/workspaces/{id}/members/sync-from-team`

#### BC-006 (Knowledge Management) との連携
- **ナレッジドキュメント共有**:
  - イベント: `KnowledgeArticlePublishedEvent`
  - アクション: ワークスペースにナレッジドキュメント共有
  - API: `POST /api/workspaces/{id}/documents/from-knowledge`
- **コラボレーティブナレッジ作成**:
  - イベント: `CollaborativeKnowledgeSessionStartedEvent`
  - アクション: 複数ユーザーでのナレッジ共同編集
  - API: `POST /api/workspaces/{id}/collaborative-sessions`

#### BC-007 L3-001 (Synchronous Communication) との連携
- **ワークスペースチャット統合**:
  - ワークスペース内リアルタイムチャット
  - ドキュメント編集中のコミュニケーション
  - @メンション通知

### API設計統合
- **ワークスペースAPI**:
  - `POST /api/workspaces` - ワークスペース作成
  - `GET /api/workspaces/{id}` - ワークスペース取得
  - `PUT /api/workspaces/{id}` - ワークスペース更新
  - `DELETE /api/workspaces/{id}` - ワークスペース削除
- **ドキュメントAPI**:
  - `POST /api/workspaces/{id}/documents` - ドキュメント作成
  - `GET /api/documents/{id}` - ドキュメント取得
  - `PUT /api/documents/{id}` - ドキュメント更新
  - `GET /api/documents/{id}/versions` - バージョン履歴
- **コラボレーションAPI**:
  - `POST /api/documents/{id}/collaborate` - 共同編集開始
  - `GET /api/documents/{id}/sessions` - 編集セッション一覧
  - `WebSocket`: `wss://api.example.com/collab/{documentId}` - リアルタイム同期
- **詳細**: [../../api/README.md](../../api/README.md)

### データ層統合
- **テーブル設計**:
  - `workspaces`: ワークスペース情報
  - `workspace_members`: メンバー・権限管理
  - `shared_documents`: ドキュメント情報
  - `document_versions`: バージョン履歴
  - `collaboration_sessions`: 編集セッション
  - `workspace_activities`: 活動ログ
- **詳細**: [../../data/README.md](../../data/README.md)

### イベント駆動統合
- **発行イベント**:
  - `WorkspaceCreatedEvent`: ワークスペース作成時
  - `DocumentSharedEvent`: ドキュメント共有時
  - `DocumentEditedEvent`: ドキュメント編集時
  - `CollaborationSessionStartedEvent`: 共同編集開始時
  - `WorkspaceMemberAddedEvent`: メンバー追加時
- **購読イベント**:
  - BC-001: `ProjectCreatedEvent`, `DeliverableCreatedEvent`
  - BC-005: `TeamCreatedEvent`, `TeamMemberAddedEvent`, `TeamMemberRemovedEvent`
  - BC-006: `KnowledgeArticlePublishedEvent`

### リアルタイムコラボレーション技術統合
- **CRDT統合**: Yjs v13+（JSON, Text, Array, Map対応）
- **WebSocket同期**: Y-WebSocket Provider（スケーラブル）
- **オフライン対応**: IndexedDB によるローカル永続化
- **競合解決**: CRDT 自動マージ（Last-Write-Wins セマンティクス）
- **カーソル同期**: Awareness Protocol（Yjs組み込み）

### セキュリティ統合
- **暗号化**: E2E暗号化（オプション、鍵管理はクライアント側）
- **認証**: JWT トークン（WebSocket handshake時検証）
- **権限管理**: RBAC（owner, editor, commenter, viewer）
- **監査ログ**: 全変更履歴の記録（削除含む）

---

## ⚙️ Operations: この能力を実現する操作

| Operation | 説明 | UseCases | V2移行元 |
|-----------|------|----------|---------|
| **OP-001**: 協働環境を提供する | ワークスペースの管理 | 1-2個 | provide-collaborative-environment |

詳細: [operations/](operations/)

---

## 📊 統計情報

- **Operations数**: 1個
- **推定UseCase数**: 1-2個
- **V2からの移行**: そのまま移行（L4候補として検討中）

---

## 🔗 V2構造への参照

> ⚠️ **移行のお知らせ**: このL3はV2構造から移行中です。
>
> **V2参照先（参照のみ）**:
> - [services/collaboration-facilitation-service/capabilities/provide-collaborative-environment/](../../../../services/collaboration-facilitation-service/capabilities/provide-collaborative-environment/)

---

## 📝 更新履歴

| 日付 | 更新内容 | 更新者 |
|------|---------|--------|
| 2025-10-31 | L3-003 README初版作成（Phase 2） | Claude |

---

**ステータス**: Phase 2 - クロスリファレンス構築中
**次のアクション**: Operationディレクトリの作成とV2からの移行
