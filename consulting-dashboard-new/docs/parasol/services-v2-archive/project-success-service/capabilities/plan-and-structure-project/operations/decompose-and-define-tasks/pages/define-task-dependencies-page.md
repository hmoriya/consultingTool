...
|---------|----------|---------|------|---------------|------|
| 先行タスク | UUID | 選択リスト | ○ | 存在するタスク | 前提となるタスク |
| 後続タスク | UUID | 選択リスト | ○ | 存在するタスク | 依存するタスク |
| 依存関係種別 | STRING_20 | 単一選択 | ○ | FS/SS/FF/SF | 依存関係のタイプ |
| リードタイム | INTEGER | 数値入力 | × | 0以上の整数 | 先行完了後の待機期間 |

### 制約条件設定
| 入力項目 | Parasol型 | 入力形式 | 必須 | バリデーション | 説明 |
|---------|----------|---------|------|---------------|------|
| 制約種別 | STRING_50 | 単一選択 | ○ | 制約タイプ一覧 | 外部・内部・リソース等 |
| 制約内容 | TEXT | テキストエリア | ○ | 最大500文字 | 具体的な制約条件 |
| 重要度 | STRING_20 | 単一選択 | ○ | 高・中・低 | 制約の重要性・影響度 |
| 期限・期間 | DATE | 日付範囲 | × | 有効な期間 | 制約の適用期間 |
