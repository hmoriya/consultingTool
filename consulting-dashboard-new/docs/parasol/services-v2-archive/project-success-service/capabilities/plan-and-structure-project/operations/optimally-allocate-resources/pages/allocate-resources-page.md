...
|---------|----------|---------|------|---------------|------|
| 最適化目標 | ARRAY<STRING_50> | 複数選択 | ○ | 目標一覧 | 効率・コスト・品質等 |
| 目標重み付け | PERCENTAGE | 数値入力 | ○ | 合計100% | 各目標の重要度（%） |
| 最適化期間 | DATE | 日付範囲 | ○ | 有効な期間 | 配分対象期間 |
| 制約強度 | INTEGER | スライダー | ○ | 1-10の範囲 | 制約の厳格さレベル |

### 配分制約条件
| 入力項目 | Parasol型 | 入力形式 | 必須 | バリデーション | 説明 |
|---------|----------|---------|------|---------------|------|
| 最大稼働率 | PERCENTAGE | 数値入力 | ○ | 0-150の範囲 | リソースの最大稼働率（%） |
| スキル適合度 | STRING_50 | 単一選択 | ○ | 完全一致/部分一致/柔軟 | スキルマッチング厳格さ |
| 分割可能性 | BOOLEAN | チェックボックス | × | - | タスクの分割・並行実行可否 |
| 優先プロジェクト | ARRAY<UUID> | 複数選択 | × | プロジェクト一覧 | 優先配分対象プロジェクト |
