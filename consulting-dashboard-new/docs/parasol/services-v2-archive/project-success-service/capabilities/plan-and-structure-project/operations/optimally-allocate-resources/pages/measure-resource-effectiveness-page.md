...
|---------|----------|---------|------|---------------|------|
| 測定期間 | DATE | 日付範囲 | ○ | 有効な期間 | 効果測定の対象期間 |
| 比較基準 | STRING_50 | 単一選択 | ○ | 基準一覧 | 改善前・業界標準・目標等 |
| 測定範囲 | ARRAY<STRING_50> | 複数選択 | ○ | 範囲一覧 | 部門・プロジェクト・チーム |
| 評価指標 | ARRAY<STRING_50> | 複数選択 | ○ | 指標一覧 | 測定対象のKPI・指標 |

### 分析パラメータ
| 入力項目 | Parasol型 | 入力形式 | 必須 | バリデーション | 説明 |
|---------|----------|---------|------|---------------|------|
| 有意水準 | DECIMAL | 数値入力 | ○ | 0.01-0.1の範囲 | 統計的有意性の水準 |
| 信頼区間 | PERCENTAGE | 数値入力 | ○ | 90-99の範囲 | 効果測定の信頼区間（%） |
| 外部要因除外 | BOOLEAN | チェックボックス | × | - | 外部要因の影響除外 |
| 遅効考慮 | INTEGER | 数値入力 | × | 0-12の範囲 | 効果発現の遅れ考慮（月） |
