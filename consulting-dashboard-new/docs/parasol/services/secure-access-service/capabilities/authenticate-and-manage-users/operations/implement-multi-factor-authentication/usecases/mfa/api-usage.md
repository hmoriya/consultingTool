...
WHAT（何ができるか）: services/secure-access-service/api/api-specification.md
...
|-----|---------------|----------|-----------|
| 認証API | POST /api/auth/authenticate | ユーザー認証実行 | `username`, `password`, `mfaToken` |
...
|---------|-----------------|---------------|----------|
| collaboration-facilitation-service | UC-COMM-01: 認証結果を通知する | 認証完了時 | ユーザーへの認証成功通知 |
...
### 記入手順
1. **MFAをリセットする**: 対応するusecase.mdのユースケース名を記入
